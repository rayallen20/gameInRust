...
- 世界坐标系: 世界坐标系用于表示物体在游戏世界中的绝对位置,通常在一个比较大的范围内使用
  - 本例中,`Player.x`记录的就是世界坐标系中的玩家水平位置
- 屏幕坐标系: 屏幕坐标系用于表示物体在屏幕上的位置,通常在一个比较小的范围内使用
  - 本例中,`Player.y`记录的就是屏幕坐标系中的玩家垂直位置

- 障碍物在屏幕坐标系下的水平位置: `障碍物的世界坐标系水平位置 - 玩家的世界坐标系水平位置`

![缺口位置示意图](./img/缺口位置示意图.png)

- 障碍物的上半部分: 从0开始,到`gap_y - size/2`
- 障碍物的下半部分: 从`gap_y + size/2`到`屏幕高度`(即`SCREEN_HEIGHT`)
...
- 在玩家和障碍物的水平坐标相同(本例中我们使用的是世界坐标系)的前提下:
  - 若玩家的垂直屏幕坐标小于障碍物的上半部分 或 大于障碍物的下半部分时
  - 则玩家与障碍物发生碰撞
...
## 3.4.4 在`play()`函数中加入障碍物和计分逻辑

- 玩家每越过一个障碍物,则:
  - 得分+1
  - 创建一个新的障碍物
- 同时,判断游戏结束的条件,除了玩家坠落到地面外,还有玩家撞到障碍物
